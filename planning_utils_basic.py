...
def prune_path_bresenham(path):
    ...
    pruned_path = [(int(p[0]), int(p[1])) for p in pruned_path]
    return pruned_path
...
def prune_path_collinearity(path, epsilon=1e-5):
    ...
    for i in range(1, len(path) - 1):
        ...
        if np.linalg.norm(np.cross(p3 - p1, p1 - p2)) / np.linalg.norm(p3 - p1) < epsilon:
            continue
...
def combined_pruning(path):
    ...
    # Ensure the pruned path contains integer coordinates
    pruned_path_combined = [(int(p[0]), int(p[1])) for p in pruned_path_combined]
